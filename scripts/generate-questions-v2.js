// Enhanced script to generate 500+ SQL practice questions with proper difficulty/topic and SQL dialects
const fs = require('fs');

const questions = [];

// Helper function to create a question
function createQuestion(id, title, difficulty, topic, description, hint, schema, seedSQL, expectedQuery, solutionSQL, expectedColumns, dialect = 'sqlite') {
  return {
    id,
    title,
    difficulty,
...
INSERT INTO products VALUES (4, 'Product D', 150);`,
  ...
  // Questions 101-130: WHERE with AND/OR
...
// ===== AGGREGATION (80 questions) =====
const aggregationQuestions = [
  // Questions 131-160: COUNT, SUM, AVG, MIN, MAX
  ...Array.from({ length: 30 }, (_, i) => {
    const num = i + 131;
...
INSERT INTO products VALUES (4, 'Product D', 400);`,
  ...
  // Questions 161-210: GROUP BY with aggregation
  ...Array.from({ length: 50 }, (_, i) => {
    const num = i + 161;
    const funcs = ['COUNT(*)', 'SUM(value)', 'AVG(value)', 'MIN(value)', 'MAX(value)'];
    const func = funcs[i % funcs.length];
...
// ===== JOINS (120 questions) =====
const joinQuestions = [
  // Questions 211-310: INNER JOIN variations
  ...Array.from({ length: 100 }, (_, i) => {
    ...
    return createQuestion(
      `q${num}`,
      `Join ${scenario.table1.charAt(0).toUpperCase() + scenario.table1.slice(1)} with ${scenario.table2.charAt(0).toUpperCase() + scenario.table2.slice(1)} #${i + 1}`,
      'medium',
      'JOINs',
...
INSERT INTO ${scenario.table1} VALUES (3, 'Record 3', 1);`,
  ...
  // Questions 311-330: LEFT JOIN
  ...Array.from({ length: 20 }, (_, i) => {
    const num = i + 311;
    return createQuestion(
      `q${num}`,
      `Left Join Customers and Orders #${i + 1}`,
      'hard',
      'JOINs',
...
// ===== SUBQUERIES (40 questions) =====
const subqueryQuestions = Array.from({ length: 40 }, (_, i) => {
  const num = i + 331;
  return createQuestion(
    `q${num}`,
...
// ===== WINDOW FUNCTIONS (50 questions) - ADVANCED =====
const windowFunctionQuestions = [
  // Questions 371-390: ROW_NUMBER
  ...Array.from({ length: 20 }, (_, i) => {
    const num = i + 371;
    return createQuestion(
      `q${num}`,
      `Row Number - Rank by Salary #${i + 1}`,
      'hard',
      'Window Functions',
      'Assign row numbers to employees ordered by salary descending.',
...
INSERT INTO employees VALUES (4, 'Diana', ${85000 + i * 1000});`,
  ...
  // Questions 391-410: RANK
  ...Array.from({ length: 20 }, (_, i) => {
    const num = i + 391;
    return createQuestion(
      `q${num}`,
      `Rank Function - Rank Employees #${i + 1}`,
      'hard',
      'Window Functions',
      'Rank employees by salary (with ties getting the same rank).',
      'Use RANK() OVER (ORDER BY ...)',
      [{ tableName: 'employees', columns: ['id', 'name', 'salary'], description: 'Employee information' }],
      `CREATE TABLE employees (id INTEGER PRIMARY KEY, name TEXT, salary INTEGER);
INSERT INTO employees VALUES (1, 'Alice', 90000);
INSERT INTO employees VALUES (2, 'Bob', 70000);
INSERT INTO employees VALUES (3, 'Charlie', 90000);
INSERT INTO employees VALUES (4, 'Diana', 85000);`,
  ...
  // Questions 411-420: DENSE_RANK
  ...Array.from({ length: 10 }, (_, i) => {
    const num = i + 411;
    return createQuestion(
      `q${num}`,
      `Dense Rank - Continuous Ranking #${i + 1}`,
      'hard',
      'Window Functions',
      'Use DENSE_RANK to rank employees without gaps in ranking.',
      'Use DENSE_RANK() OVER (ORDER BY ...)',
      [{ tableName: 'employees', columns: ['id', 'name', 'salary'], description: 'Employee information' }],
      `CREATE TABLE employees (id INTEGER PRIMARY KEY, name TEXT, salary INTEGER);
INSERT INTO employees VALUES (1, 'Alice', 90000);
INSERT INTO employees VALUES (2, 'Bob', 70000);
...
const caseQuestions = Array.from({ length: 20 }, (_, i) => {
  ...
  return createQuestion(
    `q${num}`,
    `CASE Statement - Categorize Values #${i + 1}`,
    'medium',
    'CASE Statements',
...
// ===== CTEs (30 questions) - ADVANCED =====
const cteQuestions = Array.from({ length: 30 }, (_, i) => {
  const num = i + 471;
  return createQuestion(
    `q${num}`,
...
questions.push(...windowFunctionQuestions);
questions.push(...stringQuestions);
questions.push(...caseQuestions);
questions.push(...cteQuestions);

// Generate TypeScript file
const output = `import { Question } from './questions';

// Auto-generated 500+ SQL practice questions with proper difficulty/topic and SQL dialects
...
console.log(`   - SELECT Basics: 50 (Easy)`);
console.log(`   - Filtering: 80 (Easy-Medium)`);
console.log(`   - Aggregation: 80 (Easy-Medium)`);
console.log(`   - JOINs: 120 (Medium-Hard)`);
console.log(`   - Subqueries: 40 (Hard)`);
console.log(`   - Window Functions: 50 (Hard) - ADVANCED`);
console.log(`   - String Functions: 30 (Easy)`);
console.log(`   - CASE Statements: 20 (Medium)`);
console.log(`   - CTEs: 30 (Hard) - ADVANCED`);
console.log(`   - Total: ${questions.length}`);
console.log(`\n🗄️ SQL Dialects: SQLite, MySQL, PostgreSQL`);
